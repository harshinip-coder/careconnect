...
import { EmergencyIncident, EmergencyCategory } from '../types';
...
export const ResidentDashboardScreen = ({ navigation }: any) => {
  ...
  const fetchIncidents = async () => {
    try {
      ...
      if (res.data.success || res.data.results) {
        ...
        const active = incidents.find(i => ['PENDING', 'ESCALATING', 'ACCEPTED', 'ACTIVE_RESPONSE'].includes(i.status));
  ...
  useEffect(() => {
    fetchIncidents();
    const interval = setInterval(fetchIncidents, 3000); // poll every 3s for live updates
    ...
    >
      ...
      {activeIncident ? (
        <View style={styles.activeCard}>
          ...
          {/* Chat with Responder if accepted */}
          {activeIncident.status === 'ACCEPTED' || activeIncident.status === 'ACTIVE_RESPONSE' ? (
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  profileBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#0F766E', alignItems: 'center', justifyContent: 'center' },
  ...
  chatBtn: { backgroundColor: '#16A34A', paddingVertical: 14, borderRadius: 8, alignItems: 'center', marginTop: 12 },
