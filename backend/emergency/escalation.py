...
def get_target_users_for_stage(incident, stage):
    ...
    if stage in [EscalationStage.GUARDIAN, EscalationStage.PRIMARY_GUARDIAN, EscalationStage.SECONDARY_GUARDIAN]:
        ...
        # Fallback: If resident has NO registered guardians, target all active Guardians in the system
        if not targets:
            targets = list(User.objects.filter(role=UserRole.GUARDIAN, is_active=True))
        ...
        if society:
            ...
            target_ids.update(User.objects.filter(role=UserRole.SOCIETY_MEMBER, is_active=True).values_list('id', flat=True))
...
def start_escalation_stage(incident_id, stage):
    ...
    with transaction.atomic():
        ...
        now = timezone.now()
        if not incident.global_deadline:
            incident.global_deadline = now + timedelta(minutes=15)
...
def accept_emergency_incident(incident_id, responder):
    ...
    with transaction.atomic():
        ...
        # Update escalation history record to ACCEPTED
        EmergencyEscalation.objects.filter(incident=incident, status='PENDING').update(
            status='ACCEPTED', responded_by=responder, ended_at=now
        )
